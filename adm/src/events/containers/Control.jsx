...
import EventForm from 'src/events/screens/Form'
import EventCard from 'src/events/screens/Card'
...
function Control ({ children, dismiss, data, get, save, remove, closeBtn, toggleFullBtn, id }) {
  ...
  const handleSubmit = async (edited, e) => {
    ...
    if (!id) {
      ...
      const { value } = saved
      hash.push('/events/edit/' + value._id)
    }
    else if (id) {
...
const mapStateToProps = (state, { id }) => {
  return { data: eventsDuck.selectors.get(state, { id }) }
}

const mapActionsToProps = {
  get: eventsDuck.creators.get,
  save: eventsDuck.creators.save,
  remove: eventsDuck.creators.remove,
}
