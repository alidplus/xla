...
const { ERROR, errorHandler } = require('feathers-mongoose/lib/error-handler');

exports.Service = class Service extends AdapterService {
  constructor (options) {
  ...
  _find (params = {}) {
    ...
    // Handle $populate
    if (filters.$populate) {
      q.populate({ path: filters.$populate, options: { withDeleted: true } });
    }
  ...
  _get (id, params = {}) {
    ...
    // Handle $populate
    if (filters.$populate) {
      modelQuery = modelQuery.populate({ path: filters.$populate, options: { withDeleted: true } });
    }
  ...
  _create (_data, params = {}) {
    ...
    return model.create(data, params.mongoose).then(results => {
      if ($populate) {
        return Promise.all(results.map(result => this.Model.populate(result, { path: $populate, options: { withDeleted: true } })));
      }
